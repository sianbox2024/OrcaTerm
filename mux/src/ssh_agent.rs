...
fn update_symlink<P: AsRef<Path>, Q: AsRef<Path>>(original: P, link: Q) -> anyhow::Result<()> {
    ...
    match try_once() {
        ...
        Err(err) if err.raw_os_error() == Some(1314) => {
            log::warn!(
                "skip symlink {} -> {}: missing SeCreateSymbolicLinkPrivilege; \
                 ssh agent forwarding will fall back. Enable Windows Developer Mode \
                 or grant the privilege to silence this.",
                link.display(),
                original.display()
            );
            Ok(())
