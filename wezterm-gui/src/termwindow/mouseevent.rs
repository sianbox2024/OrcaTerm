            ...
            WMEK::Press(MousePress::Left) => match item {
                ...
                TabBarItem::CopyScreenButton => {
                    ...
                    if let Some(pane) = self.get_active_pane_or_overlay() {
                        ...
                        for logical in pane.get_logical_lines(range) {
                            if !text.is_empty() {
                                text.push('\n');
                            }
                            for phys in &logical.physical_lines {
                                text.push_str(&phys.columns_as_str(0..cols));
                            }
                            // 去掉行尾补位的空白 cell
                        ...
                        // 复制成功反馈：按钮图标变 ✓ 高亮，1.5s 后恢复
                        self.copy_button_feedback = Some(Instant::now());
                        self.update_title_post_status();
                        ...
                        promise::spawn::spawn(async move {
                            ...
                            window.window.notify(TermWindowNotif::Apply(Box::new(|tw| {
                                tw.copy_button_feedback = None;
