...
impl TabBarState {
    ...
    pub fn new(
        ...
        // Config UI button
        {
            ...
            let button_start = x;
            let width = gear_button.len();
            line.append_line(gear_button.clone(), SEQ_ZERO);
            items.push(TabEntry {
                item: TabBarItem::ConfigUIButton,
                title: gear_button.clone(),
                x: button_start,
                width,
            });
            x += width;
        ...
        // Copy screen button
        {
            ...
            let button_start = x;
            let width = copy_button.len();
            line.append_line(copy_button.clone(), SEQ_ZERO);
            items.push(TabEntry {
