...
#[derive(Debug, Parser)]
#[command(
    about = "OrcaTerm - GPU accelerated terminal emulator\nBased on WezTerm (MIT) · https://github.com/wezterm/wezterm",
...
enum SubCommand {
    ...
    /// Open the orca-term graphical configuration editor
    #[command(name = "config-ui")]
    ConfigUi {
        /// Specify the configuration file to edit; defaults to the normal
        /// configuration file resolution
        #[arg(long = "config")]
        config_file: Option<OsString>,
    },
...
/// 双击启动时 Windows 会为本进程分配一个独占控制台，而启动器会驻留到 GUI 退出，
/// 黑窗全程可见。仅当控制台为本进程独占（GetConsoleProcessList 只返回自己）时
/// 隐藏它；从终端运行时控制台是共享的，不能隐藏，否则会藏掉用户的终端窗口。
/// 代价：隐藏后 GUI 启动失败的 stderr 报错不可见。
#[cfg(windows)]
fn hide_exclusively_allocated_console() {
    unsafe {
        let mut procs = [0u32; 2];
        let count = winapi::um::wincon::GetConsoleProcessList(procs.as_mut_ptr(), 2);
        if count == 1 {
...
fn main() {
    #[cfg(windows)]
    hide_exclusively_allocated_console();
...
fn run() -> anyhow::Result<()> {
        ...
        .unwrap_or_else(|| SubCommand::Start(StartCommand::default()))
    {
        ...
        SubCommand::ConfigUi { config_file } => {
            use std::process::Command;
            // ADR 0001：GUI 是独立二进制（freetype 冲突），主程序只负责拉起
            let exe_name = if cfg!(windows) {
                "orca-term-config-ui.exe"
            } else {
                "orca-term-config-ui"
            };
