...
impl ConfigInner {
    ...
    fn reload(&mut self) {
        ...
        if let Some(path) = file_name {
            ...
            // orca-term：GUI 启动时 orca-config.lua 可能尚不存在（此时走内置默认值，
            // 上面分支不成立，导致整个会话没有任何文件监听，之后配置界面创建/写入
            // 该文件也无法触发热重载）。watch 不存在的文件路径在 Windows 后端会失败
            // （错误被 .ok() 吞掉），所以监听必然存在的 exe 同目录本身；目录内出现
            // 配置文件即重载，之后按正常路径接管文件级监听。
