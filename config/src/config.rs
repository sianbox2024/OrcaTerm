...
pub struct Config {
    ...
    #[dynamic(default = "default_default_prog")]
...
";
    ...
    pub fn load_with_overrides(overrides: &wezterm_dynamic::Value) -> LoadedConfig {
        ...
        // orca-term 配置契约：只使用 exe 同目录的 orca-config.lua（存在时），
        // 不回退读取用户目录（HOME/.wezterm.lua、CONFIG_DIRS/wezterm.lua）。
    ...
    fn try_load(
        ...
        let (config, warnings) =
            wezterm_dynamic::Error::capture_warnings(|| -> anyhow::Result<Config> {
                ...
                if matches!(config, LuaValue::Nil) {
...
// orca-term 默认 shell：Windows 上优先直启 pwsh 7（跳过用户 PS 5.1 profile 的包装层，
// 避免 exit 落回 PS 5.1 提示符并在交接瞬间出现 GBK 乱码），找不到 pwsh 时回退
// powershell.exe。上游默认走 COMSPEC（cmd.exe），不符合产品预期；无配置文件首启
