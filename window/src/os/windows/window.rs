...
use crate::bitmaps::{Image, BitmapImage};
...
use winapi::um::wingdi::{
    CreateBitmap, CreateDIBSection, DeleteObject, LOGFONTW, MAKEPOINTS, BITMAPINFO,
    ...
    fn set_icon(&self, image: Image) {
        ...
        promise::spawn::spawn(async move {
            unsafe {
                ...
                if hbitmap.is_null() {
                    log::error!("Failed to create DIB section for icon");
                    return;
                }

                // Copy pixel data to the bitmap
                // Image is BGRA, Windows DIB is also BGRA in bottom-up format
                // Since we specified negative height (top-down), we copy directly
                let src_stride = width * 4;
                let dst_stride = width * 4; // 32-bit BGRA, already DWORD aligned
                for y in 0..height {
                    let src_offset = y * src_stride;
                    let dst_offset = y * dst_stride;
                    std::ptr::copy_nonoverlapping(
                        pixel_data.add(src_offset),
                        bits.add(dst_offset),
                        width * 4,
                    );
                }

                // Create mask bitmap (all zeros = no transparency mask needed for 32-bit)
                let hmask = CreateBitmap(width as i32, height as i32, 1, 1, std::ptr::null());

                let mut icon_info: ICONINFO = std::mem::zeroed();
                icon_info.fIcon = winapi::shared::minwindef::TRUE;
                ...
                if !hicon.is_null() {
                    // Set both normal and small icon
                    ...
                    // Don't destroy the icon - Windows takes ownership
                } else {
